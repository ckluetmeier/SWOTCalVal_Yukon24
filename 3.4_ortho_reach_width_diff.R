...
# pipeline (step2_run_riverobs.py), keeps the reaches listed in viable_reaches,
...
#
#   * Reach width is READ, not built here. The old script joined the SWORD node
#     shapefile for node_len, divided water area by it to get a node width, then
#     took the UNWEIGHTED MEAN of node widths in a reach. RiverObs already
#     produces a reach width, and it is not the same statistic:
#
#         width = sum(node area over observed nodes)
#                 / sum(node p_length over observed nodes)
#
#     a LENGTH-WEIGHTED mean, which is how the operational RiverSP reach product
#     defines it. Comparing a length-weighted SWOT width against an unweighted
#     orthomosaic width put a definitional difference into the residual. Expect
#     reach numbers to move slightly; node-level results are unaffected.
#
#     Consequence: the SWORD node shapefile and the `sf` dependency are gone.
#
#   * No per-cluster file list and no filename-to-date lookup. Every survey is
#     in one CSV and carries its own overpass date.
#
#   * viable_reaches is unchanged in role -- it is still the filter that decides
#     which reaches have enough orthomosaic coverage to compare -- but it is now
#     keyed by prior-database version, because a v16 reach_id does not identify
#     the same reach in v17b.
#
# CAUTION ON ortho_area_total_m2
...
REACH_CSV <- file.path(BASE, "CalVal_dataframes/width/reach/ortho_riverobs_reaches_all.csv")
OUT_CSV   <- file.path(BASE, "CalVal_dataframes/width/reach/reach_width_SWOT_Ortho.csv")

# SWOT reach timeseries, one per prior-database version. The key must equal the
# sword_version value in the ortho CSV.
SWOT_SOURCES <- c(
  v16  = file.path(BASE, "SWOT/reach/RiverSP_v16/RiverSP_domain_reach_timeseries_v16.csv"),
  v17b = file.path(BASE, "SWOT/reach/RiverSP_v17b/RiverSP_domain_reach_timeseries_PGD0_v17b.csv")
)
...
VIABLE_REACHES <- list(
  ...
  v16  = character(0)      # TODO: the v16 equivalents of the reaches above
)
...
# SURVEYS list in step2_run_riverobs.py -- a date written there as "07-10-24"
...
# NOTE: the BL list is the v16 list duplicated for v17b, because only one set
# was ever recorded. Verify the v17b ids before trusting a "BL" label there.
...
if (length(miss)) {
  stop(basename(REACH_CSV), " is missing column(s): ",
       paste(miss, collapse = ", "),
       ".\nIt should be the combined reach CSV from step2_run_riverobs.py.")
}
...
# 3. Match ortho and SWOT observations in time and space
# =============================================================================
# Same result as the old rowwise()/unnest() construction -- one row per
# (viable reach, matching overpass) -- as a join, which is faster and makes the
# match key explicit.
