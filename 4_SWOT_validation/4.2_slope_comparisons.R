...
#
# Produces: Tables 4, 5, S8, S9;  Figures 4e-f, 6b
#
# -----------------------------------------------------------------------------
# Script by:
...
# Root of the WSE and slope comparison dataframes written by scripts 1.2-2.2.
...
table4 <- slope_all %>%
  summarize_errors(SLOPE_VALUE, by = c("insitu_type", "source"),
...
tableS9 <- slope_all %>%
  summarize_errors("slope_residuals", by = c("insitu_type", "source"),
                   scale = SCALE_SLOPE, digits = DIGITS, bias_col = NULL)
print(tableS9)
...
table5 <- slope_all %>%
  ...
  summarize_errors(SLOPE_VALUE, by = c("insitu_type", "river"),
                   scale = SCALE_SLOPE, digits = DIGITS, bias_col = NULL)
print(table5)
...
# --- Figure 4e: CDF of relative reach slope by version (GNSS) -----------------
t4_gnss <- table4 %>% filter(insitu_type == "GNSS")

ggplot(filter(slope_all, insitu_type == "GNSS"),
       aes(x = abs(.data[[SLOPE_VALUE]]) * SCALE_SLOPE, color = source)) +
...
fig6b_counts <- fig6b_data %>% summarize(n = n(), .by = river)
...
walk(covariates, function(v) {
  ...
  p <- ggplot(slope_d, aes(x = abs(.data[[v]]),
    ...
    scale_color_manual(values = river_palette, breaks = river_levels, labels = river_labels) +
    labs(x = v, y = expression("Slope error (cm km"^{-1}*")"), color = "River") +
