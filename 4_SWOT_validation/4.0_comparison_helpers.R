# =============================================================================
# Shared helpers for the SWOT version C0 / D0 comparison scripts (4.1-4.6)
# -----------------------------------------------------------------------------
# Sourced by 4.1-4.6. Provides the SWORD v16 -> v17b id harmonization, the
# symmetric version partition that every comparison table is built on, the
# shared error metrics, and the river names, colors and plotting constants.
# 
# -----------------------------------------------------------------------------
# Script by:
...
#
# For the Yukon domain this affects 25 v16 nodes (each splitting into exactly 2
...
# deterministically and flag affected rows.
# -----------------------------------------------------------------------------
build_id_lut <- function(translator, from_col, to_col, label = "translator") {
  ...
  ambiguous <- lut_all %>%
    summarize(n_children = n_distinct(to_id), .by = from_id) %>%
    filter(n_children > 1)
...
# -----------------------------------------------------------------------------
# harmonize_ids() - apply the 1:1 map without changing the row count
# -----------------------------------------------------------------------------
# Adds three columns:
#   id_harmonized    v17b id as character; for v16 ids with no v17 counterpart,
#                    "v16_<original id>" so each unmatched id still counts as
#                    its own distinct feature
...
# -----------------------------------------------------------------------------
harmonize_ids <- function(df, id_col, lut, version, label = "") {
  ...
  if (version == "D") {
    out <- df %>%
      mutate(
        id_harmonized   = as.character(.data[[id_col]]),
    ...
    out <- df %>%
      ...
      mutate(
        ...
        id_harmonized   = if_else(xlate_missing,
...
# -----------------------------------------------------------------------------
# add_obs_key() - identifier for one paired observation
# -----------------------------------------------------------------------------
# Use per product:
#   node GNSS   : id_harmonized + cycle_id + pass_id + basename(drift_id)
#   node PT     : id_harmonized + pt_serial + pt_time_UTC
#                 (the node-level PT file carries no cycle_id/pass_id; pt_serial
#                  is a physical sensor and so is version-independent)
#   reach GNSS  : id_harmonized + cycle_id + pass_id + basename(drift_id)
#   reach PT    : id_harmonized + cycle_id + pass_id + pt_time_UTC
#   node width  : id_harmonized + cycle_id + pass_id + ortho id
# -----------------------------------------------------------------------------
add_obs_key <- function(df, key_cols) {
  ...
  if (!"id_harmonized" %in% key_cols) {
    warning("add_obs_key(): id_harmonized is not in key_cols. n_unique will not be ",
...
partition_versions <- function(df, key_cols, value_col, strata = "insitu_type") {
  ...
  dup <- usable %>%
    summarize(n = n(), .by = c(all_of(strata), source, obs_key)) %>%
    filter(n > 1)
  if (nrow(dup)) {
    warning(sprintf(
  ...
  obs_map <- usable %>%
    ...
    summarize(has_c = any(source == VERSION_C),
  ...
  id_map <- usable %>%
    distinct(across(all_of(strata)), id_harmonized, source) %>%
    summarize(has_c = any(source == VERSION_C),
              has_d = any(source == VERSION_D),
              .by = c(all_of(strata), id_harmonized)) %>%
    mutate(id_bucket = classify(has_c, has_d)) %>%
    select(all_of(strata), id_harmonized, id_bucket)

  out <- df %>%
    left_join(obs_map, by = c(strata, "obs_key"),       relationship = "many-to-one") %>%
    left_join(id_map,  by = c(strata, "id_harmonized"), relationship = "many-to-one") %>%
  ...
  if (n_unmap > 0) {
    message(sprintf(
      ...
      n_unmap, n_distinct(out$id_harmonized[which(out$obs_bucket == "unmappable")])))
  }
...
# -----------------------------------------------------------------------------
# summarize_errors() - metric definition
# -----------------------------------------------------------------------------
#
...
# -----------------------------------------------------------------------------
summarize_errors <- function(df, value_col, by, scale = 100, digits = 1,
                             bias_col = "bias", id_col = "id_harmonized") {
...
# -----------------------------------------------------------------------------
# partition_table() - total + the buckets
# -----------------------------------------------------------------------------
partition_table <- function(df, value_col, by = c("insitu_type", "source"),
  ...
  totals <- df %>%
    summarize_errors(value_col, by = by, scale = scale, digits = digits, bias_col = bias_col) %>%
    mutate(bucket = "total", .before = 1)

  buckets <- df %>%
    filter(!is.na(obs_bucket)) %>%
    summarize_errors(value_col, by = c(by, "obs_bucket"),
  ...
  if (any(sym$distinct_n > 1) || any(sym$distinct_n_unique > 1)) {
    ...
    stop("partition_table(): the 'same' bucket is not symmetric between versions. ",
         "Either id_harmonized is missing from key_cols, or partition_versions() ",
...
# -----------------------------------------------------------------------------
# version_change() - observation and feature counts per version
# -----------------------------------------------------------------------------
# Returns, for each version, the number of observations with a non-missing
# `value_col` and the pooled count of distinct features, together with the
# percent change from C0 to D0.
# -----------------------------------------------------------------------------
version_change <- function(df, value_col, id_col = "id_harmonized") {
  df %>%
    filter(!is.na(.data[[value_col]])) %>%
    summarize(n_obs = n(), n_unique = n_distinct(.data[[id_col]]), .by = source) %>%
...
version_colors <- c(PIC0 = "#E69F00", PGD0 = "#0072B2")
insitu_colors  <- c(PT   = "#009E73", GNSS = "#CC79A7")
