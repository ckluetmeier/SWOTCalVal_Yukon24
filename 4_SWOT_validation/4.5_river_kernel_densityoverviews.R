...
# Inputs:
#   - SWORD v17b nodes / reaches
#   - YR domain definition (node / reach IDs included in the study)
#   - SWOT RiverSP v17b PGD0 timeseries (node and reach)
#   - Hydrocron-merged SWOT node timeseries
# Per-river groupings:
#   CL, SJ, CD, PR, upperYR (single-channel), lowerYR (braided), BL
#
# Contains:
#   - Figures: width KDE by river, slope KDE by river
#   - SWOT functional repeat time stats (node & reach)
#
# -----------------------------------------------------------------------------
# Script by:
...
# AK / NA SWORD v17b node and reach shapefiles.
...
# Study domain: the SWORD node/reach IDs included in the YR analysis.
YR_DOMAIN_CSV <- file.path(DATA_ROOT, "YR_domain_v17b.csv")

# SWOT RiverSP v17b / PGD0 timeseries.
SWOT_NODE_TS  <- file.path(DATA_ROOT, "SWOT/node/RiverSP_v17b",
                           "RiverSP_domain_node_timeseries_PGD0_v17b.csv")
SWOT_REACH_TS <- file.path(DATA_ROOT, "SWOT/reach/RiverSP_v17b",
...
sword_nodes   <- st_read(SWORD_NODES)
sword_reaches <- st_read(SWORD_REACHES)

YR_domain <- read_csv(YR_DOMAIN_CSV)
...
SWOT_reach_df <- read_csv(SWOT_REACH_TS)
