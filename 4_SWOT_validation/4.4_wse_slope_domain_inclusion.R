...
# Root of the SWORD distributions (contains SWORD_v17b/).
...
# SWORD v17b node and reach shapefiles the inclusion maps are drawn on.
SWORD_NODES   <- file.path(SWORD_ROOT, "SWORD_v17b/NA",
                           "na_sword_nodes_hb81_v17b.shp")
SWORD_REACHES <- file.path(SWORD_ROOT, "SWORD_v17b/NA",
                           "na_sword_reaches_hb81_v17b.shp")

# WSE and slope comparison dataframes written by scripts 1.2-2.2.
WSE_BASE <- file.path(DATA_ROOT, "CalVal_dataframes/wse")

# Where the inclusion shapefiles are written.
INCLUSION_OUT <- file.path(DATA_ROOT, "CalVal_dataframes/inclusion_maps")

# In situ toolbox outputs that define the YR domain.
PT_NODE_DIR    <- file.path(DATA_ROOT,
                            "PTs/toolboxes_dataframes/reprocessed_2025_09_02",
                            "_node/SWORD_v17b")
PT_REACH_DIR   <- file.path(DATA_ROOT,
                            "PTs/toolboxes_dataframes/reprocessed_2025_09_02",
                            "_reach/SWORD_v17b")
GNSS_NODE_CSV  <- file.path(DATA_ROOT,
                            "GNSS/_processed_data/reprocessed_2025_09_02",
                            "SWORD_v17b/YR_drift_node_wses.csv")
GNSS_REACH_CSV <- file.path(DATA_ROOT,
                            "GNSS/_processed_data/reprocessed_2025_09_02",
                            "SWORD_v17b/YR_drift_reach_wse_slope.csv")

# SWOT node timeseries (RiverSP v17b / PGD0), used by the Figure 2 PT summary.
SWOT_NODE_TS <- file.path(DATA_ROOT, "SWOT/node/RiverSP_v17b",
                          "RiverSP_domain_node_timeseries_PGD0_v17b.csv")

# Figure and table output directories.
FIG2_DATA_DIR <- file.path(DATA_ROOT, "_figures/2_domain_map/data")
FIG3_DATA_DIR <- file.path(DATA_ROOT, "_figures/3_SWOT_examples/data")
TABLES_DIR    <- file.path(DATA_ROOT, "_figures/Tables")

# Figure 3 example inputs.
GNSS_EXAMPLE_CSV     <- file.path(FIG3_DATA_DIR, "GNSS_2024-08-20.csv")
PT_EXAMPLE_CSV       <- file.path(
  PT_NODE_DIR, "upper_PR",
  paste0("flyby_SWOTCalVal_YR_PT_L1_PT230_20240704T120000_20240821T220000_",
         "20250714T183224_SWOTCalVal_YR_KEY_20240704_20240826_v17b.csv"))
EXAMPLE_NODE_WSE_CSV <- file.path(WSE_BASE, "node/RiverTile_v17b/node_SWOT_PT.csv")
EXAMPLE_WIDTH_CSV    <- file.path(DATA_ROOT, "CalVal_dataframes/width/node",
                                  "RiverSP_v16/node_width_SWOT_Ortho.csv")

# Orthomosaic survey metadata table, read and rewritten in place by section 10.
ORTHO_SUMMARY_CSV <- file.path(TABLES_DIR, "ortho_summary_stats.csv")

# Section 7 of this script (Figure 2) writes a merged PT node file back into
...
# The data ids are coerced to character before harmonize_ids(), so the map has
...
# --- collect_domain_ids(): the in situ domain --------------------------------
...
write_inclusion_shapefile <- function(tbl, sword_path, id_name, out_path, label = "") {
  ...
  # --- nothing may exceed the shapefile field-name limit --------------
...
# --- 2a. Read, filter, harmonize (mirrors read_node() in 4.1) ----------------
...
# --- 3a. Read, filter, harmonize (mirrors read_reach() in 4.1) ---------------
...
# --- 4a. Read, filter, harmonize (mirrors read_slope() in 4.2) ---------------
...
read_reach_slope <- function(path, insitu, version) {
  ...
  d %>%
    merge_porcupine() %>%
    harmonize_ids("reach_id", reach_lut, version,
...
# -----------------------------------------------------------------------------
# Derive reach ids from node ids to count nodes instead of reaches per river.
# GNSS_df <- GNSS_df %>%
...
GNSS_stats <- GNSS_df %>%
  group_by(river) %>%
  summarize(num_drift_reaches = sum(!is.na(reach_id)), .groups = "drop")
print(GNSS_stats)
...
# PT 2159244 comes out with 7/31 as its first install time because the first
# time is parsed as NA. The two deployments are
# 2024-07-08T00:00:00.000000Z to 2024-07-26T18:50:00.000000Z and
# 2024-07-31T00:25:00.000000Z to 2024-08-21T15:35:00.000000Z, so obs_days is
# corrected to their combined length here.
...
st_write(
  PT_summary_sf,
  file.path(FIG2_DATA_DIR, "PT_summary_stats.shp"),
  delete_dsn = TRUE
)

write.csv(
  PT_summary_stats,
  file      = file.path(FIG2_DATA_DIR, "PT_summary_stats.csv"),
  row.names = FALSE
)
...
# -----------------------------------------------------------------------------
# "n_observations" is 14 characters, past the 10-character shapefile field-name
# limit, so sf abbreviates the whole attribute table of these two layers on
# write. The names are kept as they are because the Figure 2 QGIS styling is
# keyed to the on-disk field names. The same applies to PT_summary_sf above
# (pt_install_UTC, pt_uninstall_UTC, avg_pt_lat and avg_pt_lon are all over the
# limit).
...
st_write(
  ...
  file.path(FIG2_DATA_DIR, "GNSS_summary_stats_sf.shp"),
  delete_layer = TRUE
)
...
st_write(
  GNSS_summary_stats_sf,
  file.path(FIG2_DATA_DIR, "GNSS_summary_stats_node_sf.shp"),
  delete_layer = TRUE
)
...
write.csv(
  GNSS_summary_stats,
  file      = file.path(TABLES_DIR, "GNSS_summary_stats.csv"),
  row.names = FALSE
)
...
GNSS_df <- read_csv(GNSS_EXAMPLE_CSV)
...
ggplot() +
  # geom_errorbar(...)  # WSE uncertainty bars, not plotted
...
PT_df <- read_csv(PT_EXAMPLE_CSV)

example_node_SWOT_PT <- read_csv(EXAMPLE_NODE_WSE_CSV) %>%
  mutate(insitu_type = "PT") %>%
  filter(node_id == 81260300160901)

# -0.1098754 is a node-specific PT bias correction; SWOT WSE is plotted on
...
node_SWOT_ortho <- read_csv(EXAMPLE_WIDTH_CSV) %>%
  filter(river == "PR") %>%
...
ortho_df <- read_csv(ORTHO_SUMMARY_CSV) %>%
  mutate(
