...
arguments, same outputs; it just imports riverobs_shim first.
...
SEARCH-CORRIDOR FACTORS
This is where the corridor is set, because this is where the pixel-to-node
assignment happens:
 
    --wth-coef-factor 3.0 --ext-dist-coef-factor 3.0
 
equivalent to exporting RIVEROBS_WTH_COEF_FACTOR / RIVEROBS_EXT_DIST_COEF_FACTOR
and overriding them if both are set. `1.0` is stock RiverObs. See riverobs_shim.py
for what the two gates are.
 
Whatever is used is written to the RiverTile and PIXCVec as the global
attributes `riverobs_wth_coef_factor` and `riverobs_ext_dist_coef_factor`, so a
product carries a record of the corridor that produced it and step3b can report
it back to you.
=============================================================================
"""
...
# Likewise the corridor factors. These must be settled BEFORE riverobs_shim is
# imported, because the shim reads the environment at import time.
_factors = {}
...
if __name__ == '__main__':
    ...
    # Strip the one optional flag it takes so the positions hold no matter where
    # the caller put it.
    _pos, _skip = [], False
