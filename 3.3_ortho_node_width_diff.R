...
#
# WHAT CHANGED FROM THE PER-CLUSTER VERSION
#   * No per-cluster blocks to comment in and out. Every survey is processed in
#     one pass; the survey and overpass date travel with the data.
#   * ortho width is read from the pipeline, not recomputed here. RiverObs
#     already divides its summed water area by the SWORD prior node length, so
#     recomputing water_area / p_length in R would duplicate that arithmetic
#     against a p_length taken from a different table.
#   * Cloudy / edge nodes come from node_qc_all.csv rather than hard-coded
#     node_id vectors, so the exclusion is the one you drew in QGIS and it is
#     applied identically to every SWORD version.
#   * Sections 1-7 of the old script collapse into one pipeline; the old
#     section 8 (merge per-cluster CSVs) is no longer needed.
#
# Script sections:
#   0.  Configuration -- the only part you normally edit
...
ORTHO_NODES_CSV <- file.path(BASE, "CalVal_dataframes/width/node/ortho_riverobs_nodes_all.csv")
NODE_QC_CSV     <- file.path(BASE, "CalVal_dataframes/width/node/node_qc_all.csv")
...
#
SWOT_SOURCES <- c(
  v16  = file.path(BASE, "SWOT/node/hydrocron_timeseries/YR_domain_nodes_merged_RiverSP.csv"),
  v17b = file.path(BASE, "SWOT/node/RiverSP_v17b/RiverSP_domain_node_timeseries_PGD0_v17b.csv")
...
OUT_CSV <- file.path(BASE, "CalVal_dataframes/width/node/node_width_SWOT_Ortho.csv")
...
# step2_run_riverobs.py: a date written there as "07-10-24" parses in R as the
...
# 0 = same calendar day, which is what the per-cluster script did. Raise it only
# if you have checked that no second overpass falls inside the window, because
# a wider window can pair an orthomosaic with the wrong pass.
MATCH_TOLERANCE_DAYS <- 0

# --- bias grouping -----------------------------------------------------------
# The per-cluster script computed one median bias per reach-code WITHIN a single
# survey, because it only ever saw one survey at a time. c("survey",
# "sword_version", "river_code") reproduces that exactly. Dropping "survey"
# pools the two Coleen dates into a single bias -- a real change in the
# statistic, so it is a deliberate choice rather than a side effect of batching.
...
# NOTE: the BL list above is the v16 list duplicated for v17b, because the
# original script only ever recorded one set. Verify the v17b ids before
# trusting the "BL" label in a v17b run.
...
if (length(missing_ortho)) {
  stop(basename(ORTHO_NODES_CSV), " is missing column(s): ",
       ...
       ".\nIt should be the combined CSV written by step2_run_riverobs.py.")
}
...
if (nrow(date_problem) > 0) {
  ...
  stop("The overpass date did not parse to a plausible date for the survey(s) ",
       ...
       "step2_run_riverobs.py, using ISO format: 2024-07-10.")
}
...
if (max(chk$rel_err, na.rm = TRUE) > 1e-6) {
  warning(sprintf(
    ...
    "Check that the CSV came from step2_run_riverobs.py.")
}
...
if (APPLY_NODE_QC && file.exists(NODE_QC_CSV)) {
  ...
  if (n_unreviewed > 0) {
    warning(sprintf(
      "%d node(s) have no row in node_qc_all.csv and were kept. Re-run step 3c ",
      n_unreviewed),
...
read_swot <- function(path) {
  ...
  swot %>%
    ...
    mutate(
      ...
      # The overpass date used for matching comes from time_str, as in the
      # per-cluster script. time_str is the string SWOT wrote; deriving the date
      # from time_tai instead would turn any time fill value into a plausible
      # but wrong date rather than an NA that gets dropped.
...
diagnose_no_match <- function(ortho_v, swot, v) {
  ...
  message("  step2_run_riverobs.py, or set MATCH_TOLERANCE_DAYS below.")
  invisible(NULL)
}
...
# step2_run_riverobs.py, so the date filter is a per-row comparison rather than
...
#
# These come from the SURVEYS list in step2_run_riverobs.py -- confirm they are
...
matched_list <- lapply(versions, function(v) {
  ...
  out <- ortho_v %>%
    ...
    # Keep only the overpass matching each survey's own date. MATCH_TOLERANCE_DAYS
    # is 0 by default -- same-day, as the per-cluster script did.
    filter(!is.na(obs_date),
  ...
  # More than that means two overpasses share a calendar date for the same node
  # -- real (adjacent passes overlap) but it double-weights that node in every
  # statistic below, so it is reported rather than assumed away.
  dup <- out %>%
...
# 9. Data visualization
# =============================================================================
# Every plot now works on the pooled frame. Add
