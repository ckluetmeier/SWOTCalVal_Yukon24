...
#   feature_absent          the node exists but the other version's timeseries
...
#
# Section 7 repeats the whole analysis at reach scale, and section 8 emits the
# order-free percentages behind the manuscript sentence for both scales.
# =============================================================================
...
# --- reach scale --------------------------------------------------------------
# The reach cascade is NOT the node cascade. From 1.3_PT_reach_wse_slope_diff.R
# and 2.2_GNSS_reach_wse_slope_diff.R:
#
#   distinct(reach_id, time, wse)
#   reach_q  < 2          (not node_q)
#   abs(xtrk_dist) 10-60 km
#   partial_f == 0        NO NODE ANALOGUE -- rejects reaches observed over less
#                         than half their nodes, and it removes a lot: 29% of C0
#                         reach rows and 25% of D0 reach rows carry partial_f = 1
#   dark_frac <= 0.80     (1.3 uses <=, 2.2 uses <; identical in practice because
#                         4.1 then applies dark_frac < 0.50 to everything)
#
# 2.2 additionally drops time <= 0 and wse <= 0 explicitly. 1.3 does not, and it
# makes no difference: every fill-WSE reach row carries reach_q = 3 (294 of 1935
# rows in C0, 264 of 1923 in D0), so reach_q < 2 removes them all either way.
# The assertion in section 9 checks this rather than assuming it.
...
REACH_VALUE <- "residuals_nobias"   # 4.1 partitions reach WSE on the same metric
...
# v17b ids that have a v16 ancestor. A D0-only observation on a feature outside
# these sets is on a feature that is new in v17b.
...
# half, reach ids for the reach half. Defaulting it silently to the node set made
# every reach D0-only observation come back as "no_sword_counterpart".
...
# 7. REACH SCALE
# =============================================================================
# The same analysis at reach scale. Three things differ from the node half:
#
#   1. The cascade uses reach_q, and adds partial_f == 0 -- a filter with no node
#      analogue that rejects reaches observed over less than half their nodes.
#   2. BOTH observation keys carry cycle_id and pass_id (4.1 keys reach PT on
#      id + cycle + pass + pt_time_UTC), so the lookup is a direct join for PT as
#      well as GNSS. No time matching is needed at this scale.
#   3. reach_q_b is NOT node_q_b. The two differ at bits 0, 4, 9, 15, 23, 24 and
#      25 -- bit 25 is below_min_fit_points for reaches and no_sig0_observations
#      for nodes -- so REACH_Q_B_LABELS is used here, never NODE_Q_B_LABELS.
#
# 4.1 also excludes three sub-9 km reaches per SWORD version; that exclusion is
# applied here so the rebuild can reproduce Table S6's reach half.

# --- 7a. rebuild the 4.1 reach partition -------------------------------------
# KEEP IN SYNC WITH read_reach() in 4.1.
...
message("[4.6 reach] failed_qc rows, each filter failed at all (ORDER-FREE -- quote these):")
...
# Grouped by `group` only, NOT by obs_bucket: the control rows come from "same"
# observations and so carry no C0_only/D0_only direction. Splitting the contrast
# by direction would leave the control in a bucket of its own and compare it
# against nothing.
...
write_csv(tableS6_reach_rebuilt, file.path(OUT, "tableS6_reach_rebuilt.csv"))
write_csv(reach_why_all,         file.path(OUT, "version_unique_reach_obs_reasons.csv"))
write_csv(table9_reasons,        file.path(OUT, "reach_reason_counts.csv"))
write_csv(table9_reason_features,file.path(OUT, "reach_reason_counts_with_reaches.csv"))
write_csv(table10_binding,       file.path(OUT, "reach_failed_qc_binding_filter.csv"))
write_csv(table10_anyfail,       file.path(OUT, "reach_failed_qc_filter_failures.csv"))
write_csv(table11_reach_bits,    file.path(OUT, "reach_q_b_bit_prevalence.csv"))
write_csv(table11_degraded,      file.path(OUT, "reach_q_b_degraded_bits.csv"))
write_csv(table12_sentence,      file.path(OUT, "sentence_numbers.csv"))
