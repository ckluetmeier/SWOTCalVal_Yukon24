...
Usage:
    python 3.1.2_run_RiverObs.py \\
        ...
        --raster-dir /path/rasterized_water_masks \\
        --rdf-dir . \\
        --out-dir /path/RiverObs_output

The surveys processed are the entries of the SURVEYS list below;
...
# Surveys One entry per orthomosaic; uncomment an entry to include it in
# the run. `date` is the matching SWOT overpass date (UTC) and is
# carried through to the output CSV, so the downstream temporal match is
# a join condition rather than a hand-edited constant.
#
# `date` is the SWOT OVERPASS date, not the flight date, and must be ISO
# YYYY-MM-DD. It is copied verbatim into the SWOT_date column of the
# combined CSVs, where downstream code parses it as a date -- '07-10-24'
# parses in R as the year 7 and silently matches nothing. Validated at
# startup below.
SURVEYS = [
...
# Node fields worth keeping. `width` is the ortho width: RiverObs
# computed it as node area / SWORD p_length using the same
# RiverNode.width_area() call the operational processor uses for
# RiverSP.
NODE_FIELDS = [
...
# Reach fields. `width` here is sum(node area) / sum(node p_length) over
# the observed nodes -- a length-weighted mean, not the unweighted mean
# of node widths. `obs_frac_n` is the fraction of the reach's SWORD
# nodes that received data, and is the defensible basis for a
# reach-completeness threshold.
REACH_FIELDS = [
...
def run_one(survey, version, rdf, args):
    ...
    if not os.path.exists(water_tif):
        # FileNotFoundError, not SystemExit: SystemExit does not inherit
        # from Exception, so --keep-going cannot catch it and a single
        # missing raster would end the batch before any CSV is written.
    ...
    # run_calval2rivertile.py, not src/bin/calval2rivertile.py -- the
    # wrapper loads riverobs_shim first. See riverobs_shim.py for what
    # it patches.
    cmd = [
        sys.executable,
    ...
    # Corridor factors, if given, are forwarded rather than relied on
    # through the environment, so the exact command printed below is
    # reproducible.
    ...
    # RiverObs writes 'width'; rename so downstream code cannot confuse
    # the reference width with the SWOT width column.
...
def main():
    ...
    p.add_argument('--raster-dir', required=True,
                   help='output directory from 3.1.1_rasterize_watermask.py')
    ...
    p.add_argument('--res', type=float, default=3.0,
                   help='must match the --res used in 3.1.1_rasterize_watermask.py')
    ...
    # Catch a malformed overpass date here, where it is one line to fix,
    # rather than downstream where it turns into an empty join with no
    # error.
    bad = []
    for sv in SURVEYS:
        try:
