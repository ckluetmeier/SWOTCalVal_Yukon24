#!/usr/bin/env python
"""Check that a SWORD netCDF is usable by RiverObs.
...
# Variables RiverObs reads to build the centerline, size the search
# corridor, and convert area to width. `type` is NOT here -- missing is
# fine, RiverObs derives it from reach_id.
ASSIGNMENT_CRITICAL = {
...
# Subgroups ReachDatabaseReaches.__call__ dereferences directly. These
# are NOT covered by the masked-array fallback -- .subset()/.__call__()
# get invoked on them, so they must genuinely exist.
